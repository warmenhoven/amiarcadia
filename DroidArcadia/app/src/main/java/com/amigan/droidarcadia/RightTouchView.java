...
public class RightTouchView extends View
{   private SparseArray<PointF> mActivePointers;
    ...
    public RightTouchView(Context context, AttributeSet attrs)
    {   super(context, attrs);
        initView();
    }
    ...
    {   int pointerIndex = event.getActionIndex();           // get pointer index from the event object
        ...
        {   // add new pointer to list of pointers
            ...
            {   checkfingers(1); // right is paddle
            } else
            {   checkfingers(0); // right is keypad
            }
    ...
    {   super.onDraw(canvas);
        ...
        if (!MainActivity.handedness) // right is keypad
        {   for (qi = 0; qi < 12; qi++)
