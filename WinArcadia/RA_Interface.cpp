  ...
  DownloadFunc fnDownload, void* pDownloadUserData, DWORD* pBytesRead, DWORD* pStatusCode)
{
    ...
    else
    {
        ...
        if (nTemp == nHostnameLen) // "If the return value [of mbstowcs()] is count [(ie. 3rd argument to mbstowcs())], the wide-character string is not null-terminated."
        {   wBuffer[nTemp] = L'\0';
        }
#endif
        if (nTemp > 0)
        ...
        else
        {
            ...
            if (nTemp == strlen(sRequestedPage) + 1) // "If the return value [of mbstowcs()] is count [(ie. 3rd argument to mbstowcs())], the wide-character string is not null-terminated."
            {   wBuffer[nTemp] = L'\0';
            }
#endif
...
static const char* CCONV _RA_InstallIntegration()
{
    ...
    {
        char buffer[1024];
        sprintf(buffer, "Could not load RA_Integration.dll: %d\n%s\n", ::GetLastError(), GetLastErrorAsString().c_str());
...
static UHUGE ParseVersion(const char* sVersion)
{
    char* pPart;

    UHUGE major = strtoul(sVersion, &pPart, 10);
    if (*pPart == '.')
        ++pPart;

    UHUGE minor = strtoul(pPart, &pPart, 10);
    if (*pPart == '.')
        ++pPart;

    UHUGE patch = strtoul(pPart, &pPart, 10);
    if (*pPart == '.')
        ++pPart;

    UHUGE revision = strtoul(pPart, &pPart, 10);

    // 64-bit max signed value is 9223 37203 68547 75807
    UHUGE version = (major * 100000) + minor;
...
static void RA_InitCommon(HWND hMainHWND, int nEmulatorID, const char* sClientName, const char* sClientVersion)
{
    ...
    {
      sprintf(sHostUrl, "http://%s", _RA_HostName());
    }
    ...
    if (DoBlockingHttpCallWithRetry(sHostUrl, "dorequest.php", "r=latestintegration", buffer, sizeof(buffer), &nBytesRead, &nStatusCode) == FALSE)
    {
        ...
        {
            sprintf(buffer, "Cannot access %s (status code %u)\nWorking offline.", sHostUrl, nStatusCode);
    ...
    *ptr = '\0';
    ...
    const UHUGE nMinimumDLLVer = ParseVersion(sVersionBuffer);

    GetJsonField(buffer, "LatestVersion", sVersionBuffer, sizeof(sVersionBuffer));
    const UHUGE nLatestDLLVer = ParseVersion(sVersionBuffer);
    ...
    if (nLatestDLLVer == 0 || !sLatestVersionUrl[0])
    {
        ...
        if (sLatestVersionUrl[0])
            sprintf(buffer, "Failed to fetch latest integration version.\n\n%s", sLatestVersionUrl);
        else
            sprintf(buffer, "The latest integration check did not return a valid response.");
    ...
    int nMBReply = 0;
    UHUGE nVerInstalled = ParseVersion(sVerInstalled);
    if (nVerInstalled < nMinimumDLLVer)
    {
        ...
        if (nVerInstalled == 0)
        {
            sprintf(buffer, "Install RetroAchievements toolset?\n\n"
                "In order to earn achievements you must download the toolset library.");
        }
        else
        {
            sprintf(buffer, "Upgrade RetroAchievements toolset?\n\n"
    ...
    if (nMBReply == IDYES)
    {
        ...
        {
            sprintf(buffer, "Failed to update toolset (status code %u).", nStatusCode);
            MessageBoxA(hMainHWND, buffer, "Error", MB_OK | MB_ICONERROR);
        }
    }
    ...
    if (nVerInstalled < nMinimumDLLVer)
    {
        ...
        sprintf(buffer, "%s toolset is required to earn achievements.", nVerInstalled == 0 ? "The" : "A newer");
        MessageBoxA(hMainHWND, buffer, "Warning", MB_OK | MB_ICONWARNING);
    }
